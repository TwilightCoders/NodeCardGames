$(document).ready(function()
{
  ...
  var app =
  {
    init: function()
    {
      // Assign 'this' to 'self' so that it can be used in a callback function
      var self = this;
      
      // Add event listener to number of players button
      $("input[name='numPlayers']").change(function()
      {
        var numPlayers = parseInt($(this).val());
        self.configurePlayerNames(numPlayers);
      });
    },
...
var game =
{
  ...
  init: function(settings)
  {
    ...
    {
      return; // Stop the game! We need some settings to format the game with
    }
    
    //
    this.initialValues.players = settings.players;
